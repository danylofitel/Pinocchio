...
\subsection{Updated Classification Algorithm}
...
In case of naive approach only probabilities of separate features need to be stored. They are sufficient to determine the overall probability of the feature vector for all $2^n$ combinations, regardless of whether they are present in the training set. In the approach described above, however, only the information about concrete feature vectors from the training set is stored. While it is possible to calculate probability of any feature vector in the training set by the algorithm above, in its current version probability of any feature vector that is not in the training is $0$.

This can be interpreted as excessive 'formality', i.e. the algorithm produces the optimal result, but in the context of given training set. We would naturally want to get a reasonable approximation of the probability of messages that are not present in the training set. Note that out of $2^n$ possible feature vectors at most $k$ will be present in the constructed trie, and the probability that the algorithm will produce the expected result is $\frac{k}{2^n}$ at worst. Therefore, providing an alternative probability for unrecognized feature vectors is essential.

One possible way is to fall back to naive Bayesian classifier for all feature vectors that are not in the training set. In this case, our modified classifier will only produce different results for a small fraction of possible messages. However, if the number of features $n$ is small, i.e. of order $\log_2 k$, the probability of any feature vector to be in the training set will be $1 - (\frac{k-1}{k})^k$, which is close to $1$ for large training sets.

The downside of small number of features is that only fraction of available information is used. In practice we want feature vectors to include as many words as possible. Therefore, we should provide a different approximation of $P(x | c)$.

\newpage

\section{Semi-Naive Bayesian Classification}

At this point we have an optimal algorithm for any message such that its feature vector is present in the training set. In this section we will describe a number of ways to give an approximation of probabilities for all possible feature vectors.
...
Using tries for storing intermediate values of $M(c, x_1, \dots, x_i)$ allows to perform lookup of these values in $O(n)$, therefore, computing $P(x | c)$ takes $O(\sum_{i=1}^{n} n) = O(n^2)$. This might be good enough in practice for relatively small values of $n$. Another benefit is that it does not depend on the size of the training set, therefore the quality of the classifier can be improved by increasing the training set without impacting classification performance.

TODO: philosophy about zero conditional probabilities in the chain.

TODO: updated training and classification algorithms.

TODO: computation with limited depth.

TODO: computation with limited number of conditions.
