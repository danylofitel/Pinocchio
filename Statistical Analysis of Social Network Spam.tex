...
Consider the feature vector $x = (x_1, x_2, \dots, x_n), x_i = $, $x_i = 1$ if the message contains $w_i$, $x_i = 0$ otherwise. As mentioned before, calculating the value for all $2^n$ possible values of $x$ is not feasible. However, note that we can express this probability in terms of conditional probabilities using the chain rule:

$$P(x | c) = P(x_1 | x_2, x_3, \dots, x_n, c) * P(x_2 | x_3, x_4, \dots, x_n, c) * \dots * P(x_n | c) = \prod_{i=1}^{n}P(x_i | \bigcap_{j = i + 1}^{n} x_j, c)$$

Computing all probabilities in the product above is not feasible either as it will lead to the same exponential complexity of the algorithm. However, we can still improve the classifier precision compared to naive version by calculating conditional probabilities for some of the features while using a priori values for the rest. The next step is to determine the features a posteriori probabilities of which differ most from a priori values.
