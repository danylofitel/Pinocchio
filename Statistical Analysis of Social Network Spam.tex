...
Of course, this is not the only way to store and perform lookup on feature vectors. We can also use a hash table. Construction and lookup are $O(nk)$ and $O(n)$ on average respectively, however they can be $O(n^2k)$ and $O(n^2)$ in the worst case. In this paper we will focus on trie approach.
...
Alternatively, we can express this probability in terms of conditional probabilities using the chain rule:
...
Let $M(c, x_1, x_2, \dots, x_l), 1 \leq l \leq n$ denote the subset of training set of legitimate messages if $c = L$ or spam messages if $c = S$ with the first $l$ features equal to $x_1, x_2, \dots, x_l$. Then
...
Computing and storing  probabilities of each component of the product is not possible since the number conditions grows exponentially. The number of combinations for $i$-th component is $2^{i}$, and the total number of probabilities that need to be stored is $\sum_{i=1}^{n} 2^i = 2^{i+1} - 1$. Therefore, these values have to be computed during classification process.
