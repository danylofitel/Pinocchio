...
Let $N = 1099$ denote the total number of training messages, $N_L$ - the number of legitimate messages and $N_S$ - the number of spam messages. When comparing performance of classifiers, we will be interested in the following characteristics:
...
\begin{itemize}
	...
	\item Error rate: $E = \frac{N_{L \rightarrow S}}{N}$ - the number of legitimate messages classified as spam.
	\item Legitimate message fallout: $F_L = \frac{N_{L \rightarrow S}}{N_L}$ - the number of legitimate messages classified as spam.
	\item Spam message fallout: $F_S = \frac{N_{S \rightarrow L}}{N_S}$ - the number of legitimate messages classified as spam.
\end{itemize}


Because of significantly higher requirements to storage ($O(kn)$ instead of $O(n)$) the non-naive Bayesian classifier was not tested with all 21611 possible features. Instead, it was tested with with a number of configurations with smaller number of features.
