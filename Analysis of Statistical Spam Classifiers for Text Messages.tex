...
The messages come in preprocessed form containing only the words from subject and body, each word is encoded with an integer. The database allows to choose among 4 flavors: the original messages, messages processed with stop-list (100 most frequent English words removed), messages processed with the  lemmatizer (each word converted to the base form) and finally the version with both lemmatizer and stop-list applied. In practice the database processed with both lemmatizer and stop-list performs best for most of the algorithms, so we will only use this version. It contains 21611 distinct words which we will use as features for modified Bayesian classifier.
...
In this thesis we analyzed a number of common statistical classifier of text spam. For naive Bayesian classifier we proposed a number of improvements that tackle formally incorrect assumptions about feature independence. The resulting classifier performed better than the naive classifier with comparable speed, but additional storage.

When compared to other approaches, Bayesian classifier does produce better results than k nearest neighbors, however it is usually behind the perceptron and SVM. It still has a number of benefits. Firstly, Bayesian classifier allows to easily tweak the relative cost of false positives, which can be a critical requirement in spam filtering. Secondly, the quality of classification only improves with larger training sets, when for linear neural net classifiers it might cause problems with overfitting.
